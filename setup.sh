...
# Handles Bun setup, then hands off to the Bun setup modules.
...
check_bun() {
  ...
  if command -v bun >/dev/null 2>&1; then
    ...
    log "Bun $BUN_VERSION at $BUN_PATH_FOUND (major=$major, ok=$BUN_OK)"
  else
    log "Bun not found"
  fi
}
...
install_deps() {
  ...
  log "Running bun install $bun_flags"
  if bun install $bun_flags >> "$LOG_FILE" 2>&1; then
    DEPS_OK="true"
  ...
  # Verify SQLite is available through Bun's runtime.
  log "Verifying Bun SQLite runtime"
  if bun --eval 'import { Database } from "bun:sqlite"; const db = new Database(":memory:"); db.exec("SELECT 1"); db.close();' >> "$LOG_FILE" 2>&1; then
    NATIVE_OK="true"
    log "Bun SQLite runtime is available"
  else
    log "Bun SQLite runtime failed to initialize"
  fi
}
